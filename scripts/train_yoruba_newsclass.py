...
from dataloaders.yoruba_newsclass_topic import YorubaNewsclassDataModule
...
if __name__ == "__main__":
    ...
    with open(
        "../conf/experiment_config_yoruba.yaml",
        "r",
    ) as f:
    ...
    TRAIN_DATAPATH = data["train-datapath"]
    ...
    for idx, params in enumerate(permutations_dicts):
        ...
        # set the seed for dataset generation
        ...
        early_stop_callback = CustomEarlyStopping(
            min_steps=25, monitor="val_f1", min_delta=0.00, patience=3, verbose=False, mode="max"
        )
        ...
        name = name = str(params["seed"]) + "test"
        ...
        if params["adv-penalty"]:
            name = "Adv_" + name
        project = f"yoruba_{params['num-labeled-examples']}_examples"
