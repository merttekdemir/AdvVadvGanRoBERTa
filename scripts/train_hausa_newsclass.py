...
if __name__ == "__main__":
    ...
    with open(
        "../conf/experiment_config_hausa.yaml",
        "r",
    ) as f:
    ...
    for idx, params in enumerate(permutations_dicts):
        ...
        early_stop_callback = CustomEarlyStopping(
            min_steps=250, monitor="val_f1", min_delta=0.00, patience=3, verbose=False, mode="max"
        )
        ...
        if params["adv-penalty"]:
            name = "Adv_" + name
        project = f"hausa_{params['num-labeled-examples']}_examples"
