...
class HausaNewsclassDataset(Dataset):
...
class HausaNewsclassDataModule(pl.LightningDataModule):
    def __init__(
        self,
        batch_size: int,
        ...
        if split == "train":
            if df is None:
                ...
                df = self.download_data(
                    dataset="mtek2000/hausa_newsclass_topic",
    ...
    def setup(self, stage: str) -> None:
        ...
        """
        if stage == "fit":
            self.train = HausaNewsclassDataset(self.train)
            self.val = HausaNewsclassDataset(self.test)

        if stage == "few_shot":
            self.train = HausaNewsclassDataset(self.few_shot)
            self.val = HausaNewsclassDataset(self.test)

        if stage == "test":
            self.test = HausaNewsclassDataset(self.test)

        if stage == "predict":
            self.predict = HausaNewsclassDataset(self.test)
